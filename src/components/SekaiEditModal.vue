  ...
  >
    ...
    >
      <a-form-item
        ...
        name="SEKAIID"
      >
        <a-input v-model:value="formState.SEKAIID" placeholder="请输入SEKAI ID" :disabled="true" />
      </a-form-item>
...
// 监听sekaiData属性变化
watch(() => props.sekaiData, (newVal) => {
  if (newVal && Object.keys(newVal).length > 0) {
    formState.value = {
      shareId: newVal.id,
      SEKAIID: newVal.SEKAIID || newVal.sekaiId,
      sekaiName: newVal['MySEKAI昵称ID'] || newVal.sekaiName,
      serverRegionId: newVal.ServerRegion?.serverid,
      description: newVal['MySEKAI描述'] || newVal.description,
      fixtures: (newVal.SekaiFixtures || []).map(item => item['物品id'] || item.fixtureId)
    }
  }
}, { immediate: true })
...
const handleOk = async () => {
  try {
    ...
    const response = await api.post('/api/update_sekai_info', {
      shareId: formState.value.shareId,
